...
data = pd.read_csv('porus_2K', sep='\\s+', header=None)
x_y = data.iloc[:, :2].values
X, Y = x_y[:, 0], x_y[:, 1]

anomaly_mask = (0.4 <= X) & (X <= 0.6) & (0.4 <= Y) & (Y <= 0.6)
background_mask = ~anomaly_mask

K = torch.tensor(np.where(anomaly_mask, 0.10, 0.05), dtype=torch.float32).to(DEVICE)
...
def loss_function(model):
    ...
    x_coords = x[:, 0]
    y_coords = x[:, 1]
    anomaly_local = (0.4 <= x_coords) & (x_coords <= 0.6) & (0.4 <= y_coords) & (y_coords <= 0.6)
    bg_local = ~anomaly_local
    
    pde1 = (res[anomaly_local].pow(2).mean() if anomaly_local.sum() > 0 else 0) + \
...
def train_forward_pinn():
    ...
    torch.save(model.state_dict(), "forward_output_2K_run1.pth")
    
    with torch.no_grad():
        preds = model(interior_pts).cpu().numpy()
    
    out = np.hstack((x_y, preds))
    np.savetxt("forward_output_2K_run1.csv", out, delimiter=",", header="x,y,u_x,u_y,p", comments="")
