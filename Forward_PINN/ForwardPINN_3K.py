...
data = pd.read_csv('porus_3K', sep='\\s+', header=None)
x_y = data.iloc[:, :2].values
X, Y = x_y[:, 0], x_y[:, 1]

mask1 = (0.1 <= X) & (X <= 0.3) & (0.1 <= Y) & (Y <= 0.3)
mask2 = (0.7 <= X) & (X <= 0.9) & (0.7 <= Y) & (Y <= 0.9)
background_mask = ~(mask1 | mask2)

K = torch.tensor(np.where(mask1 | mask2, 0.10, 0.05), dtype=torch.float32).to(DEVICE)
...
def loss_function(model):
    ...
    x_coords = x[:, 0]
    y_coords = x[:, 1]
    mask1_local = (0.1 <= x_coords) & (x_coords <= 0.3) & (0.1 <= y_coords) & (y_coords <= 0.3)
    mask2_local = (0.7 <= x_coords) & (x_coords <= 0.9) & (0.7 <= y_coords) & (y_coords <= 0.9)
    bg_local = ~(mask1_local | mask2_local)
    
    pde1 = (res[mask1_local].pow(2).mean() if mask1_local.sum() > 0 else 0) + \
           (res[mask2_local].pow(2).mean() if mask2_local.sum() > 0 else 0) + \
           (res[bg_local].pow(2).mean() if bg_local.sum() > 0 else 0)
...
def train_forward_pinn():
    ...
    print(f"\nPhase A completed in {time.time() - start_adam:.1f}s")
    ...
    for block in range(4000 // 100): 
        ...
        print(f"L-BFGS Loss: {loss_lbfgs:.3e}")

    print(f"\nPhase B completed in {time.time() - start_lbfgs:.1f}s")

    torch.save(model.state_dict(), "forward_output_3K_run1.pth")
    
    with torch.no_grad():
        preds = model(interior_pts).cpu().numpy()
    
    out = np.hstack((x_y, preds))
    np.savetxt("forward_output_3K_run1.csv", out, delimiter=",", header="x,y,u_x,u_y,p", comments="")
