...
data = pd.read_csv("forward_output_1K_run1.csv")
x_y = data[["x", "y"]].values
...
def loss_function(model: InversePINN):
    ...
    grad_K = autograd.grad(K_pred.sum(), x, create_graph=True)[0]
    
    l2_reg = torch.mean(grad_K[:, 0] ** 2 + grad_K[:, 1] ** 2)
    
    if current_epoch < 2000:
        l2_weight = 0.0
        
    elif current_epoch < 4000:
        progress = (current_epoch - 2000) / 2000
        l2_weight = 2e-4 * progress
        
    elif current_epoch < 6000:
        l2_weight = 4e-4
        
    elif current_epoch < 8000:
        l2_weight = 6e-4
        
    else:
        l2_weight = 8e-4
    
    total_reg = l2_weight * l2_reg
...
def train_inverse_pinn():
    ...
    os.makedirs("ckpts_l2", exist_ok=True)
    BEST_CKPT = "ckpts_l2/BEST_MODEL.pth"
    best_loss = float("inf")
    ...
    torch.save(model.state_dict(), "inverse_output_1K_L2_run1.pth")
    ...
    np.savetxt("inverse_output_1K_L2_run1.csv", out, delimiter=",", header="x,y,u_x,u_y,p,K", comments="")
    ...
    print(f"\n  Naturally discovered low K:  {centers[0]:.6f}")
    print(f"  Naturally discovered high K: {centers[1]:.6f}")
