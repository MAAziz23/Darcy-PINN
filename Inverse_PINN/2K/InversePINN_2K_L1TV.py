...
def loss_function(model: InversePINN):
    ...
    l1_reg = torch.mean(torch.abs(grad_K[:, 0]) + torch.abs(grad_K[:, 1]))
    
    tv_reg = torch.mean(torch.sqrt(grad_K[:, 0] ** 2 + grad_K[:, 1] ** 2 + 1e-8))
    ...
    if current_epoch < 2000:
        ...
        l1_weight = 1e-5 * progress
        tv_weight = 2e-4 * progress
    ...
    total_reg = l1_weight * l1_reg + tv_weight * tv_reg
...
def train_inverse_pinn():
    ...
    os.makedirs("ckpts_2kl1tv", exist_ok=True)
    BEST_CKPT = "ckpts_2kl1tv/BEST_MODEL.pth"
    best_loss = float("inf")
    ...
    torch.save(model.state_dict(), "inverse_output_2K_L1TV_run1.pth")
    ...
    np.savetxt("inverse_output_2K_L1TV_run1.csv", out, delimiter=",", header="x,y,u_x,u_y,p,K", comments="")
